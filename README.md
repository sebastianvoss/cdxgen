...
| BAZEL_USE_ACTION_GRAPH       | If you need to create a SBOM for a specific Bazel target set this to `true` and cdxgen will use `bazel aquery 'outputs(".*.jar", deps(<BAZEL_TARGET>))'` (defaults to `false`) |
